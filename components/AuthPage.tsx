...
const AuthPage: React.FC<AuthPageProps> = ({ onClose, onLoginSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[130] bg-black flex overflow-hidden animate-fade-in">
      ...
      <div className="hidden lg:flex w-1/2 bg-[#02142C] relative flex-col items-center justify-center p-24 overflow-hidden">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-radial-gradient from-[#76ABB8]/10 to-transparent blur-[120px]"></div>
        
        <div className="relative z-10 flex flex-col items-center text-center space-y-8 animate-slide-up">
          <LogoIcon animate={isLoading} />
          <div className="space-y-4">
            <h2 className="text-6xl font-black text-white tracking-tighter uppercase leading-none">
              {isLoading ? 'Processing...' : (isSuccess ? 'Verified' : 'Build the Future')}
            </h2>
          ...
          <div className="flex gap-4 pt-8">
            <div className="px-6 py-2 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-white/40 tracking-widest uppercase">Incubation</div>
            <div className="px-6 py-2 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-white/40 tracking-widest uppercase">Mentorship</div>
            <div className="px-6 py-2 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-white/40 tracking-widest uppercase">Capital</div>
          </div>
        </div>
      </div>
